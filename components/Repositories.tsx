      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-4 w-full gap-2 items-center mt-4">
          {data.map((item: Repositories, index) => (
            <Link key={index} href={"https://github.com/" + item.full_name} target="_blank" className="h-full bg-zinc-900/90 hover:bg-zinc-200/20 relative transition-all duration-200 flex flex-col rounded-lg py-4 px-5" data-aos="fade-up">
               <h1 className="leading-none text-lg text-zinc-100">
                <span className="text-primary text-xl font-medium">{item.owner.login}</span>/{(item.owner.login.length + item.name.length + 1) > 33 ? (
              <>{item.name.slice(0, 33 - (item.owner.login.length + item.name.length + 1))}<br/>{item.name.slice(33  - (item.owner.login.length + item.name.length + 1), item.name.length)}</>): item.name}
              </h1>
              <p className="line-clamp-2 mt-2 h-12">{item.description ? item.description : "No description."}</p>
              <div className="mt-3 flex justify-between">
